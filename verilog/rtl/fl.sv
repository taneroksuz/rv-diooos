...
module fl (
  ...
  fl_reg_type r, rin, v;
  logic [FL_IDX_BITS-1:0] alloc_slot0;
  logic [FL_IDX_BITS-1:0] alloc_slot1;
  ...
  always_comb begin
    ...
    if (flush) begin
      ...
      if (fl_in.free_en0 && (v.spec_count < FL_CNT_BITS'(FLIST_DEPTH))) begin
        v.do_free0                   = 1'b1;
        v.free0_slot                 = v.tail[FL_IDX_BITS-1:0];
        v.tail                       = v.tail + 1'b1;
        v.spec_count                 = v.spec_count + 1'b1;
        v.comm_head                  = v.comm_head + 1'b1;
        v.list_written[v.free0_slot] = 1'b1;
      end

      if (fl_in.free_en1 && (v.spec_count < FL_CNT_BITS'(FLIST_DEPTH))) begin
        v.do_free1                   = 1'b1;
        v.free1_slot                 = v.tail[FL_IDX_BITS-1:0];
        v.tail                       = v.tail + 1'b1;
        v.spec_count                 = v.spec_count + 1'b1;
        v.comm_head                  = v.comm_head + 1'b1;
        v.list_written[v.free1_slot] = 1'b1;
      end
  ...
  always_ff @(posedge clock) begin
    if (reset != 0) begin
      if (rin.do_free0) begin
        list[rin.free0_slot] <= fl_in.free_tag0;
      end
      if (rin.do_free1) begin
        list[rin.free1_slot] <= fl_in.free_tag1;
      end
    end
  end
endmodule
